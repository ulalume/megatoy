...
/**
 * The envelope graph in megatoy's own terms.
 *
 * ym2612_eg::graph decides everything about the drawing -- the two traces, the
 * width of the time axis, where a sounding voice sits on it, and how often a
 * curve may be rebuilt -- from ym2612_eg::OperatorParams. The two things it
 * cannot know are here: how megatoy's register struct maps onto those params,
 * and the single note every graph is drawn at.
 */
