 ...
 * is recorded silently. No stored version means a fresh install or a build
 * predating the change log, which are indistinguishable here, so both are
 * announced and the caller words the notice for both.
 *
 * The caller records the version whatever it decides, or a user who ignores
 * the notice gets it again every launch.
 */
